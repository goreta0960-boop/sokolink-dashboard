...
type Listing = { id: string; product_name: string; qty: number; unit: string; price_tzs: number; status: string };
...
export default function SupplierDashboard() {
  ...
  async function loadData(supplierId: string) {
    const [ordersRes, listingsRes, ratingsRes] = await Promise.all([
      ...
      supabase.from("inventory").select("id, product_name, qty, unit, price_tzs, status")
        .eq("supplier_id", supplierId).order("created_at", { ascending: false }).limit(20),
      ...
      <div style={styles.grid}>
        ...
        <section style={styles.section}>
          ...
          <table style={styles.table}>
            <thead>
              <tr>{["Product", "Qty", "Price TZS", "Status"].map((h) => <th key={h} style={styles.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {listings.map((l) => (
                <tr key={l.id}>
